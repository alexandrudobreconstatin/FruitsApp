...
class FruitsService
{
   ...
   * @throws Exception
   */
  public function fetchAndSaveFruits(): void
  {
    ...
    $subject = "Fruit data has been loaded and logs have been saved";
    $body = "Information on the number of records added and updated: " . json_encode($counters);
    $this->addToEmailQueue($subject, $body);
  }
  ...
  public function addToEmailQueue(string $subject, string $body): void
  {
    $emailQueue = new EmailQueue();

    $emailQueue->setEmailFrom('appsFruits@test.com');
    $emailQueue->setEmailTo('alexdoobre93@gmail.com');
    $emailQueue->setEmailSubject($subject);
    $emailQueue->setEmailContent($body);
    $emailQueue->setStatus(0); // Statusul inițial pentru email-urile care urmează să fie trimise
    $emailQueue->setDateAdded(new \DateTime());
